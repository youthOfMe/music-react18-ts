...
class XHRequest {
    ...
    request<T = any>(config: XHRequestConfig<T>) {
        // 单次请求的成功拦截处理
        config = config.interceptors?.requestSuccessFunc ? config.interceptors.requestSuccessFunc(config) : config

        return new Promise<T>((resolve, reject) => {
            this.instance.request<any, T>(config).then(res => {
                res = config.interceptors?.responseSuccessFunc ? config.interceptors.responseSuccessFunc(res) : res
                resolve(res)
            }).catch(err => {
                reject(err)
            })
